---
title: "Node Setup"
description: "Complete guide to setting up a Zilliqa 2.0 node and joining mainnet, testnet, or devnet"
---
...
<Note>
We don't recommend to run Nodes behind a NAT, if you're doing so
and you are facing any traversal issue you might have to debug on your own.
</Note>
...
7. Generate the startup script and the configuration file for your node by running:
  ...
  ```
  <Note>You can replace zq2-mainnet with `zq2-testnet` or `zq2-devnet` depending on
  which network you want your node to join.</Note>
...
8. (Optional) A Zilliqa node contains various performance and operational metrics compatible with the OpenTelemetry 
  ...
  ```
  <Note>For more details on testing and using the available OpenTelemetry 
  metrics refer to the [OpenTelemetry](/node-operations/nodes/monitoring/opentelemetry) page.</Note>
...
9. Generate the node private key.
  ...
  ```
  <Note>Please save the node key as described above. You may need it
  in the future to restart the node to generate the BLS public
  key of the node.</Note>
...
<Tabs>
  <Tab title="Synchronization from a checkpoint">
    This method leverages a predefined checkpoint block number and hash and the corresponding state imported from a checkpoint file. Historical states based on blocks prior to the checkpoint are unavailable. Before proceeding to the [start the node section](/node-operations/nodes/node#starting-your-node), configure the checkpoint settings according to the instructions in [syncing-from-checkpoints](/node-operations/nodes/checkpoints).
  </Tab>
  <Tab title="Synchronization from the genesis">
...
Please refer to [Syncing & Pruning](/node-operations/nodes/passive-pruning) for information on how to download or discard historical blocks.

### Starting your node
Since only devnet nodes can sync from the genesis, all other nodes must be started from a checkpoint: 

<Tabs>
  <Tab title="Start from checkpoint">
    ```bash
    chmod +x start_node.sh && \
    ./start_node.sh -k $PRIVATE_KEY -p <checkpoint_block_num.dat>
    ```
  </Tab>
  <Tab title="Start from genesis">
    ```bash
    chmod +x start_node.sh && \
...
<Note>After a node is successfully launched from a checkpoint for the first time, the checkpoint settings can be removed from its configuration file and the node can be restarted without specifying a checkpoint file on the command line.</Note>

<Note>The `<checkpoint_block_num.dat>` file is the one you previously downloaded. Refer to [syncing-from-checkpoint](/node-operations/nodes/checkpoints#syncing-a-node-from-a-checkpoint)</Note>
...
z2 join --chain zq2-mainnet
```
<Note>Replace `zq2-mainnet` with the chain you are running on.</Note>
