...
description: "Learn how to create and manage a staking pool with validator nodes on Zilliqa 2.0"
---
...
<Tabs>
  <Tab title="Node with Deposited Stake">
  ...
  <Tab title="Node Without Deposited Stake">
    ...
    ```
    <Note>`<DELEGATION_CONTRACT_PROXY_ADDRESS>` must be set as the value of `control_address`
    in the JSON input passed to `convert-key`.</Note>
